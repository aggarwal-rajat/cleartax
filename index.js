...
function populateSection(category) {
    ...
    category.rows.forEach(row => {
        cotainerClub.appendChild(populateRow(row, category.maxRowCapacity, category.price));
    });
    return cotainerClub;
}

function populateRow(row, maxSeats, price) {
    ...
    for (let i = maxSeats; i > maxSeats - row.seats; i--) {
        ...
        seatBox.setAttribute("seat-price", price);
...
function updateSeats() {
    ...
    if (ScreenData.selectedSeats.indexOf(selectedSeats) > -1) {
        ...
        if (currentSelection.length == numberOfSeats) {
            ...
            const oldItem = document.getElementById(firstItem);
            oldItem.classList.remove("selected");
            totalPrice -= parseInt(oldItem.getAttribute("seat-price"))
        }
        if (index > -1) {
            ...
            totalPrice += parseInt(this.getAttribute("seat-price"))
        }
    }
